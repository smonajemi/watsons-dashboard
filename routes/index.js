...
router.get('/getMenu/:option', (req, res) => {
  ...
  gfs.files.findOne({metadata: param}, {sort: {uploadDate: -1}}, (err, file) => {
...
try {
  ...
  res.render('success', {title: "Dashboard",user: req.session.user, menu: req.file.originalname, uploadedFile: metadata, isMenu: true})
})
